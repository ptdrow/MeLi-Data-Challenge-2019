...
# -*- coding: utf-8 -*-
"""
Created on Fri Sep 27 23:14:02 2019
...
#os.chdir('/home/ptdrow/Documents/Machine_Learning/MeLi_Classifier/')
import wording
from ptKmeans import ptKMeans
...
def decider1(macro_category, words_ids, vocabulary_p, category_dict, categories_p, i):
    selector = kmeans.selector[i][macro_category]
    if len(words_ids) > 0:
        decider = np.prod(vocabulary_p[selector, words_ids],axis=1) * categories_p[selector]
        return(category_dict[np.argmax(decider)])
    else: return('UNDECIDED')

def classify_title(row, vocabulary, vocabulary_p, 
                   decider,stopwords, 
                   category_dict, categories_p, i):
    macro_prediction = kmeans.model[i].predict(kmeans.vectorizer[i].transform([row.title]))
    ...
    return decider(macro_prediction[0],words_ids, vocabulary_p, category_dict, categories_p, i)
...
subset_data(load_data(), train_r = 0.2, test_r = 0.0075)
kmeans = ptKMeans(PATH, version,318,200)
kmeans.fit()
